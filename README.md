...
(Добавь компонент <Tooltip> </Tooltip> В требуемое место, для пояснения. ) Пример на заголовке страницы Home:
   ```jsx
        <Tooltip text={'Люди, которые организуют все это.'} >
            <h2>Организаторы</h2>
        </Tooltip>
   ```
*Не забудь импортировать сверху файла Home.js*
